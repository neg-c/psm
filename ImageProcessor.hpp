...
static Mat3f rgb2lcc(Mat3f src) {
...
static Mat3f lcc2rgb(Mat3f lcc) {
  Eigen::Matrix3f transform_mat;
  // clang-format off
...
static Mat4f scaleTo4d(Mat3f lcc, const Eigen::MatrixXf& alpha) {
...
static double convertToRGBangle(double theta) {
  if (theta < M_PI / 3) {
...
static double convertToORGBangle(double theta) {
  if (theta < M_PI / 2) {
...
static Mat3f switch_rb(Mat3f src) {
...
static Mat3f lcc2orgb(Mat3f lcc) {
...
static Mat3f orgb2lcc(const Mat3f& orgb) {

  ...
  if (s.image.is_loaded && s.image.is_processed) {
    ...
    if (mouse_pos.x >= image_pos.x &&
      ...
      if (pixel_x >= 0 && pixel_x < s.image.width && pixel_y >= 0 &&
        ...
        if (s.image.is_16bit) {
          const auto& display_data = std::get<std::vector<uint16_t>>(s.image.display_data);
          ...
          const auto& display_data = std::get<std::vector<unsigned char>>(s.image.display_data);
          if (idx + 2 < display_data.size()) {
            s.pixel.setColor(display_data[idx],
                             display_data[idx + 1],
                             display_data[idx + 2]);
...
void PreviewArea::DrawMagnifyingGlass(AppState& s, GLuint texture_id,
  ...
  draw_list->AddRect(center_min, center_max, IM_COL32(255, 255, 0, 255), 0.0f, 0,
                     2.0f);
}
